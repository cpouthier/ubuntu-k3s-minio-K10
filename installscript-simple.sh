...
chmod +x $HOME/minio-binaries/mc
sudo mv $HOME/minio-binaries/mc /usr/local/bin/mc
...
cat <<EOF >>minio-standard-profile.yaml
...
kubectl apply -f minio-standard-profile.yaml

#Create Location profile for Minio Standard bucket
cat <<EOF >>minio-immutable-profile.yaml
apiVersion: config.kio.kasten.io/v1alpha1
kind: Profile
metadata:
...
kubectl apply -f minio-immutable-profile.yaml

# Enable Kasten daily Kasten reports

cat <<EOF >>reports.yaml
kind: Policy
apiVersion: config.kio.kasten.io/v1alpha1
metadata:
...
cat > pacman-backup-policy.yaml <<EOF
apiVersion: config.kio.kasten.io/v1alpha1
kind: Policy
metadata:
...
kubectl apply -f pacman-backup-policy.yaml
...
Minio console is available on  http://$get_ip:9001, with the same username/password.
    ...
    Both of the can be accessed through API on http://$get_ip:9000 using credentials ($username/$password)
...
echo "    Both of the can be accessed through API on http://$get_ip:9000 using credentials ($username/$password)"
